...
describe('E2E Role Admin', () => {
    ...
    beforeAll(async () => {
        ...
        successData = {
            name: 'testRole1',
            permissions: permissions.map((val) => `${val.id}`),
    ...
    it(`GET ${E2E_PERMISSION_ADMIN_GET_URL} Get Success`, async () => {
        const response = await request(app.getHttpServer())
            .get(E2E_PERMISSION_ADMIN_GET_URL.replace(':id', role.id))
            .set('Authorization', `Bearer ${accessToken}`)
            .set('x-permission-token', permissionToken);
    ...
    it(`POST ${E2E_PERMISSION_ADMIN_CREATE_URL} Create Error Request`, async () => {
        const response = await request(app.getHttpServer())
    ...
    it(`POST ${E2E_PERMISSION_ADMIN_CREATE_URL} Create Exist`, async () => {
        const response = await request(app.getHttpServer())
            .post(E2E_PERMISSION_ADMIN_CREATE_URL)
            .send(existData)
            .set('Authorization', `Bearer ${accessToken}`)
    ...
    it(`POST ${E2E_PERMISSION_ADMIN_CREATE_URL} Create Permission Not Found`, async () => {
        const response = await request(app.getHttpServer())
            .post(E2E_PERMISSION_ADMIN_CREATE_URL)
            .send({
                ...successData,
    ...
    it(`POST ${E2E_PERMISSION_ADMIN_CREATE_URL} Create Success`, async () => {
        const response = await request(app.getHttpServer())
            .post(E2E_PERMISSION_ADMIN_CREATE_URL)
            .send(successData)
            .set('Authorization', `Bearer ${accessToken}`)
    ...
    it(`PUT ${E2E_PERMISSION_ADMIN_UPDATE_URL} Update Exist`, async () => {
        const response = await request(app.getHttpServer())
            .put(E2E_PERMISSION_ADMIN_UPDATE_URL.replace(':id', roleUpdate.id))
            .send(existData)
            .set('Authorization', `Bearer ${accessToken}`)
    ...
    it(`DELETE ${E2E_PERMISSION_ADMIN_DELETE_URL} Delete Not Found`, async () => {
        const response = await request(app.getHttpServer())
            .delete(
    ...
    it(`DELETE ${E2E_PERMISSION_ADMIN_DELETE_URL} Delete Success`, async () => {
        const response = await request(app.getHttpServer())
            .delete(E2E_PERMISSION_ADMIN_DELETE_URL.replace(':id', role.id))
            .set('Authorization', `Bearer ${accessToken}`)
            .set('x-permission-token', permissionToken);
